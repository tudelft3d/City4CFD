...
namespace roofer::reconstruction {
    ...
    FaceInfo operator()(const FaceInfo a, const FaceInfo b) const {
      ...
      if (a.segid != 0 && b.segid == 0) {
        ...
        r = b;
                 ...
                 b.segid != 0) {  // we need to merge 2 faces with a plane
  ...
  typedef CGAL::Arr_face_overlay_traits<Arrangement_2, Arrangement_2,
                                        Arrangement_2, overlay_functor>
      Overlay_traits;
  ...
  class Face_index_observer : public CGAL::Arr_observer<Arrangement_2> {
   ...
   public:
    Face_index_observer(Arrangement_2& arr, bool is_footprint, size_t pid,
                        float elevation, Plane plane)
        : CGAL::Arr_observer<Arrangement_2>(arr),
          n_faces(0),
          in_footprint(is_footprint),
          plane_id(pid),
          elevation(elevation),
          plane(plane) {
   ...
   public:
    ...
    virtual void after_split_face(Face_handle old_face, Face_handle new_face,
      ...
      if (n_faces == 1)
        ...
        if (!is_hole &&
            hole_mode) {  // detect case where a `hole` is added that touches
                          // surrounding outer_ccb (in one vertex)
    ...
    void set_hole_mode(bool mode) { hole_mode = mode; }
  };
    ...
    virtual void before_merge_face(Face_handle remaining_face,
      ...
      if (sum_count != 0) {
        ...
        remaining_face->data().elevation_50p =
            remaining_face->data().elevation_50p * w1 +
            discarded_face->data().elevation_50p * w2;
        remaining_face->data().elevation_70p =
            remaining_face->data().elevation_70p * w1 +
            discarded_face->data().elevation_70p * w2;
        remaining_face->data().elevation_97p =
            std::max(remaining_face->data().elevation_97p,
                     discarded_face->data().elevation_97p);
        remaining_face->data().data_coverage =
            remaining_face->data().data_coverage * w1 +
            discarded_face->data().data_coverage * w2;
        // and sum the counts
        remaining_face->data().pixel_count = sum_count;
      }
      remaining_face->data().elevation_min =
          std::min(remaining_face->data().elevation_min,
                   discarded_face->data().elevation_min);
      remaining_face->data().elevation_max =
          std::max(remaining_face->data().elevation_max,
                   discarded_face->data().elevation_max);
      // merge the point lists
      // if (remaining_face==discarded_face){
      //   std::cout << "merging the same face!?\n";
      //   return;
      // }
      // remaining_face->data().points.insert(remaining_face->data().points.end(),
      // discarded_face->data().points.begin(),
      // discarded_face->data().points.end() );
    }
  };
  ...
  class Snap_observer : public CGAL::Arr_observer<Arrangement_2> {
   public:
    ...
    virtual void after_split_face(Face_handle old_face, Face_handle new_face,
                                  bool) {
      if (old_face->data().segid == 0) new_face->set_data(old_face->data());
    }
  };
  ...
  bool arrangementface_to_polygon(Face_handle face,
                                  roofer::LinearRing& polygons, double h = 0);
