...
namespace roofer::io {
  ...
  class VectorWriterOGR : public VectorWriterInterface {
    ...
    inline void create_field(OGRLayer* layer, const std::string& name,
    ...
    OGRPolygon create_polygon(const LinearRing& lr) {
      ...
      for (auto& g : lr) {
        ...
        ogrring.addPoint(coord_t[0], coord_t[1], coord_t[2]);
      }
      ogrring.closeRings();
      ...
      for (auto& iring : lr.interior_rings()) {
        ...
        for (auto& g : iring) {
          auto coord_t = pjHelper.coord_transform_rev(g[0], g[1], g[2]);
          ogr_iring.addPoint(coord_t[0], coord_t[1], coord_t[2]);
        }
   ...
   public:
    ...
    void writePolygons(const std::string& source,
      ...
      dataSource = (GDALDataset*)GDALOpenEx(
          connstr.c_str(), GDAL_OF_VECTOR | GDAL_OF_UPDATE, NULL, NULL, NULL);
      if (dataSource == nullptr) {
        dataSource =
            driver->Create(connstr.c_str(), 0, 0, 0, GDT_Unknown, NULL);
      }
      if (dataSource == nullptr) {
      ...
      // } else if
      // (geom_term.is_connected_type(typeid(std::vector<TriangleCollection>))
      // || geom_term.is_connected_type(typeid(MultiTriangleCollection))) {
      //   // Note that in case of a MultiTriangleCollection we actually write
      //   the
      //   // TriangleCollections separately, and not the whole
      //   MultiTriangleCollection
      //   // to a single feature. That's why a MultiPolygon and not an
      //   aggregate of
      //   // multipolygons.
      //   wkbType = wkbMultiPolygon25D;
      // } else if (geom_term.is_connected_type(typeid(std::unordered_map<int,
      // Mesh>))) {
      //   wkbType = wkbMultiPolygon25D;
      // }
      ...
      if (overwrite_layer_) {  // FileGDB does not support OVERWRITE, and falls
                               // back to OpenFileGDB (no multipatch support)
                               // when appending
        lco = CSLSetNameValue(lco, "OVERWRITE", "YES");
      } else {
        lco = CSLSetNameValue(lco, "OVERWRITE", "NO");
        layer = dataSource->GetLayerByName(
            find_and_replace(layername, "-", "_").c_str());
      }

      // bool supports_list_attributes = gdaldriver != "ESRI Shapefile" &&
      // gdaldriver != "FileGDB";
      ...
      if (layer == nullptr) {
        ...
        for (auto& [name, vecvar] : attributes.get_attributes()) {
          ...
          if (auto vec = attributes.get_if<bool>(name)) {
            ...
            // if (total_size != vec->size()) throw(rooferException("Number of
            // attributes not equal to number of geometries [field name =" +
            // name + "]"));
        ...
        for (auto& [name, vec] : attributes.get_attributes()) {
          ...
          //   throw(rooferException("Number of attributes not equal to number
          //   of geometries [field name =" + name + "]"));
          // }
      ...
      for (size_t i = begin; i != end; ++i) {
        ...
        // create a vec of features for the case where we write multiple feature
        // rows (building with multiple parts)
        ...
        for (auto& [name, varvec] : attributes.get_attributes()) {
          ...
          if (auto vec = attributes.get_if<bool>(name)) {
            ...
            if ((*vec)[i].has_value()) {
              poFeature->SetField(attr_id_map[name], (*vec)[i].value().year,
                                  (*vec)[i].value().month,
                                  (*vec)[i].value().day);
            } else {
              poFeature->SetFieldNull(attr_id_map[name]);
            ...
            if ((*vec)[i].has_value()) {
              poFeature->SetField(
                  attr_id_map[name], 0, 0, 0, (*vec)[i].value().hour,
                  (*vec)[i].value().minute, (*vec)[i].value().second,
                  (*vec)[i].value().timeZone);
            } else {
              poFeature->SetFieldNull(attr_id_map[name]);
            }
          } else if (auto vec = attributes.get_if<DateTime>(name)) {
            if ((*vec)[i].has_value()) {
              poFeature->SetField(
                  attr_id_map[name], (*vec)[i].value().date.year,
                  (*vec)[i].value().date.month, (*vec)[i].value().date.day,
                  (*vec)[i].value().time.hour, (*vec)[i].value().time.minute,
                  (*vec)[i].value().time.second,
                  (*vec)[i].value().time.timeZone);
            } else {
              poFeature->SetFieldNull(attr_id_map[name]);
            }
          }
        ...
        // Cast the incoming geometry to the appropriate GDAL type. Note that
        // this need to be in line with what is set for wkbType above.
        if (!polygons[i].size()) {
          ...
          // } else {
          //   std::cerr << "Unsupported type of input geometry " <<
          //   geom_term.get_connected_type().name() << std::endl;
          // }
        }
        ...
        if (i % transaction_batch_size_ == 0) {
          if (do_transactions_)
            if (dataSource->CommitTransaction() != OGRERR_NONE) {
              throw(
                  rooferException("Committing features to database failed.\n"));
            }
          if (do_transactions_)
            if (dataSource->StartTransaction() != OGRERR_NONE) {
              throw(rooferException("Starting database transaction failed.\n"));
...
//                 auto coord_t = pjHelper.coord_transform_rev(vertex[0],
//                 vertex[1], vertex[2]); ring.addPoint(coord_t[0],
//                               coord_t[1],
//                               coord_t[2]);
//               }
//               ring.closeRings();
...
//                 poFeature_->SetFieldNull(attr_id_map[attr_map.first]); else {
//                   // Since the 'attribute_value' type is a 'variant' and
//                   therefore
//                   // the 'attr_map' AttributeMap is a vector of variants, the
//                   // SetField method does not recognize the data type stored
//                   // within the variant. So it doesn't write the values
//                   unless we
//                   // put the values into an array with an explicit type. I
//                   tried
//                   // passing attr_map.second.data() to SetField but doesn't
//                   work. attribute_value v = attr_map.second[0]; if
//                   (std::holds_alternative<int>(v)) {
//                     std::vector<int> val(attr_map.second.size());
//                     for (size_t h=0; h<attr_map.second.size(); h++) {
//                       val[h] = std::get<int>(attr_map.second[h]);
//                     }
//                     poFeature_->SetField(attr_id_map[attr_map.first],
//                     attr_map.second.size(), val.data());
//                   }
...
//                     // FIXME: needs to align the character encoding with the
//                     encoding of the database, otherwise will throw an 'ERROR:
//                     invalid byte sequence for encoding ...'
//   //                  const char* val[attr_map.second.size()];
//   //                  for (size_t h=0; h<attr_map.second.size(); h++) {
//   //                    val[h] =
//   std::get<std::string>(attr_map.second[h]).c_str();
//   //                  }
//   //                  poFeature_->SetField(attr_id_map[attr_map.first],
//   attr_map.second.size(), val);
//                   }
...
//                     attr_map.second.size(), val.data());
//                   }
//                   else throw(rooferException("Unsupported attribute value
//                   type for: " + attr_map.first));
//                 }
//               }

//               auto bp_id = std::to_string(mtcs.building_part_ids_[j]);
//               poFeature_->SetField(attr_id_map["building_part_id"],
//               bp_id.c_str());
//             }
//             poFeatures.push_back(poFeature_);
//           }
...
//         } else if (geom_term.is_connected_type(typeid(std::unordered_map<int,
//         Mesh>))) {
//           const auto& meshes = geom_term.get<std::unordered_map<int,
//           Mesh>>(i);

//           for ( const auto& [mid, mesh] :
//           geom_term.get<std::unordered_map<int, Mesh>>(i) ) {
//             auto poFeature_ = poFeature->Clone();
...
//               val = mesh.get_labels();
//               poFeature_->SetField(attr_id_map["labels"], label_size,
//               val.data());
//             }
