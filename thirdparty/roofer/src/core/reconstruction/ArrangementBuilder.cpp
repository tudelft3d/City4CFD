...
namespace roofer::reconstruction {
  ...
  class ArrangementBuilder : public ArrangementBuilderInterface {
    Arrangement_2::Vertex_handle arr_cross_edge(
        Arrangement_2& arr, Segment_2& segment,
        Arrangement_2::Halfedge_handle& edge, const double& dist_threshold) {
      ...
      // std::cout << "\ts degree=" << edge->source()->degree() << ", t degree="
      // << edge->target()->degree() << "\n" << std::endl;
      ...
      if (result) {
        if (const Point_2* p = std::get_if<Point_2>(&*result)) {
          ...
          // CGAL::to_double(p->y())+CGAL::to_double(geo_p.y()) << std::endl;
          // check if point of intersection is practically at the same
          // coordinates of one of the edge's end points
...
// std::cout << "\tsplitting the edge at " << *p << std::endl;
#ifdef DEBUGGIE
            std::cout << "POINT(" << CGAL::to_double(p->x()) << " "
                      << CGAL::to_double(p->y()) << ")\n";
#endif
            auto e = arr.split_edge(edge, Segment_2(e_source, *p),
                                    Segment_2(*p, e_target));
            return e->target();  // e->target is the split vertex
          }
        }
      } else {
#ifdef DEBUGGIE
        std::cout << "#ERROR arr_cross_edge: no intersection!!!!!!"
                  << std::endl;
#endif
      }
    ...
    bool arr_edge_exists(Arrangement_2& arr,
                         const Arrangement_2::Vertex_handle& s,
                         const Arrangement_2::Vertex_handle& t) {
    ...
    void arr_insert(Arrangement_2& arr, Segment_2 segment,
      ...
      if (zone_elems.size() == 0) {
        ...
        // collect the vertices in the crossing of segment with the arrangement.
        // Some are existing, some are created in this function.
        std::list<Arrangement_2::Vertex_handle> vertices;
        // std::pair<Arrangement_2::Vertex_handle, Arrangement_2::Vertex_handle>
        // vertices;
        ...
        if (assign(face, zone_elems.front())) {
          if (zone_elems.size() == 1) {
            ...
            std::cout << "POINT(" << CGAL::to_double(p_source.x()) << " "
                      << CGAL::to_double(p_source.y()) << ")\n";
            std::cout << "LINESTRING(" << CGAL::to_double(p_source.x()) << " "
                      << CGAL::to_double(p_source.y()) << ", ";
            std::cout << CGAL::to_double(vertex->point().x()) << " "
                      << CGAL::to_double(vertex->point().y()) << ")\n";
#endif
            arr.insert_from_right_vertex(Segment_2(p_source, vertex->point()),
                                         vertex);
        ...
        while (zone_elems.size()) {
          // for a face we only are left with handling the case were face is the
          // last element in the zone, otherwise skip faces
          if (assign(face, zone_elems.front()) && (zone_elems.size() == 1)) {
            auto& v = vertices.front();

#ifdef DEBUGGIE
            std::cout << "#inserting last edge in face vertices.size()=="
                      << vertices.size() << "\n";
            std::cout << "#insert_from_left_vertex..." << std::endl;
            std::cout << "POINT(" << CGAL::to_double(p_target.x()) << " "
                      << CGAL::to_double(p_target.y()) << ")\n";
            std::cout << "LINESTRING(" << CGAL::to_double(vertex->point().x())
                      << " " << CGAL::to_double(vertex->point().y()) << ", ";
            std::cout << CGAL::to_double(p_target.x()) << " "
                      << CGAL::to_double(p_target.y()) << ")\n";
#endif
            ...
            vertices.push_back(
                arr_cross_edge(arr, segment, edge, dist_threshold));
          ...
          if (vertices.size() == 2) {
            ...
            // s = arr_overlap_correction(arr, s, t, common_face,
            // dist_threshold); t = arr_overlap_correction(arr, t, s,
            // common_face, dist_threshold);
            if (s == t) {
#ifdef DEBUGGIE
...
// "\n" << std::endl;
#ifdef DEBUGGIE
              std::cout << "LINESTRING(" << CGAL::to_double(s->point().x())
                        << " " << CGAL::to_double(s->point().y()) << ", ";
              std::cout << CGAL::to_double(t->point().x()) << " "
                        << CGAL::to_double(t->point().y()) << ")\n";
#endif
    ...
    void arr_extend_insert_segment(Arrangement_2& arr, Segment_2 segment,
                                   const float& extension) {
      auto lv = segment.to_vector();
      lv = lv / CGAL::sqrt(CGAL::to_double(lv.squared_length()));
#ifdef DEBUGGIE
      std::cout << "LINESTRING("
                << CGAL::to_double((segment.source() - lv * extension).x())
                << " "
                << CGAL::to_double((segment.source() - lv * extension).y())
                << ", ";
      std::cout << CGAL::to_double((segment.target() + lv * extension).x())
                << " "
                << CGAL::to_double((segment.target() + lv * extension).y())
                << ")\n";
#endif
      insert(arr, Segment_2(segment.source() - lv * extension,
                            segment.target() + lv * extension));
    }

   public:
    void compute(Arrangement_2& arrangement, LinearRing& footprint_,
                 std::vector<EPECK::Segment_2>& input_edges,
                 ArrangementBuilderConfig cfg) override {
...
// prepare footprint segments
      ...
      {
        // insert(arr_base, footprint.outer_boundary().edges_begin(),
        // footprint.outer_boundary().edges_end()); arr_insert_polygon(arr_base,
        // footprint); insert_non_intersecting_curves(arr_base,
        // footprint.edges_begin(), footprint.edges_end()); if
        // (!footprint.outer_boundary().is_simple()) {
      ...
      //       std::sort(segments.begin(), segments.end(), [](PointPair& a,
      //       PointPair& b) {
      //         return CGAL::squared_distance(a.first,a.second) >
      //         CGAL::squared_distance(b.first,b.second);
      //       });
      //     }
      ...
      // if (snap_clean) arr_snapclean(arrangement, snap_dist,
      // snap_detect_only);
    }
  };
