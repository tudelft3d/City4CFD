...
namespace roofer {
  ...
  namespace tri_util {
     ...
     * level by 1. facets in the domain are those with an odd nesting level.
     */
    ...
    void insert_ring(roofer::vec3f& ring, CDT& cdt) {
      ...
      CDT::Vertex_handle vh_next, vh_last,
          vh = cdt.insert(K::Point_2((*pit_last)[0], (*pit_last)[1]));
      vh_last = vh;
    ...
    CDT create_from_polygon(roofer::LinearRing& poly) {
      ...
      if (triangulation.number_of_faces() == 0) return triangulation;
  ...
  namespace proj_tri_util {
    DT cdt_from_linearing(const roofer::LinearRing& poly) {
      ...
      DT cdt;
      for (auto& p : poly) cdt.insert(Point_3(p[0], p[1], p[2]));
      for (auto& ring : poly.interior_rings())
    ...
    float interpolate_from_cdt(const Point_2& p, const DT& cdt) {
      ...
      if (lt == DT::OUTSIDE_CONVEX_HULL) {
        ...
        if (!cdt.is_infinite(fh->vertex(0)))
          interp_edge.push_back(fh->vertex(0)->point());
        if (!cdt.is_infinite(fh->vertex(1)))
          interp_edge.push_back(fh->vertex(1)->point());
        if (!cdt.is_infinite(fh->vertex(2)))
          interp_edge.push_back(fh->vertex(2)->point());
        ...
        double g1 =
            CGAL::approximate_sqrt(CGAL::squared_distance(pt1, pt_int)) /
            CGAL::approximate_sqrt(CGAL::squared_distance(pt1, pt2));

        float h_int =
            interp_edge[0].z() + g1 * (interp_edge[1].z() - interp_edge[0].z());
      ...
      CGAL::Barycentric_coordinates::triangle_coordinates_2(
          EPICK::Point_2(fh->vertex(0)->point().x(),
                         fh->vertex(0)->point().y()),
          EPICK::Point_2(fh->vertex(1)->point().x(),
                         fh->vertex(1)->point().y()),
          EPICK::Point_2(fh->vertex(2)->point().x(),
                         fh->vertex(2)->point().y()),
          EPICK::Point_2(pt.x(), pt.y()), std::back_inserter(coords));
