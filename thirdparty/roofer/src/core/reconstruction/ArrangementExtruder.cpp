...
namespace roofer::reconstruction {
  ...
  inline arr3f v2p(Arrangement_2::Vertex_handle v, float h) {
    return {float(CGAL::to_double(v->point().x())),
            float(CGAL::to_double(v->point().y())), h};
  }
  template <typename P>
  arr3f p2p(P p) {
  ...
  vec3f get_heights(std::vector<hf_pair>& vertex_column, Vertex_handle v,
                    Face_handle f_a, Face_handle f_b, float& h_a, float& h_b) {
    vec3f v_other;
    float h_prev = -999999;
    ...
    for (auto& [h, face] : vertex_column) {
      if (face == f_a) {
        ...
        if (h != h_prev) v_other.push_back(v2p(v, h));
      }  // else break;
      h_prev = h;
    }
    return v_other;
  ...
  class ArrangementExtruder : public ArrangementExtruderInterface {
   public:
    void compute(Arrangement_2& arr,
      ...
      unbounded_face->data().elevation_97p =
      ...
      if (cfg.generate_floor) {
        ...
        for (Arrangement_2::Hole_iterator floorpart_ =
                 unbounded_face->holes_begin();
             floorpart_ != unbounded_face->holes_end(); ++floorpart_) {
          LinearRing floor;
          auto he = *floorpart_;
          auto first = he;
          do {
        ...
        for (auto face : arr.face_handles()) {
          if (face->data().is_footprint_hole) {
            ...
            if (hole.size() != 0 &&
                multisolid.find(part_id) != multisolid.end()) {
              auto& floor = multisolid[part_id].get_polygons()[0];
              floor.interior_rings().push_back(hole);
            } else {
              std::cout << "skipping a hole for which no polygon exists...\n";
            }
          }
        }
      }

      // check for no detected planes (if one face inside fp has none then all
      // have none)
      ...
      for (auto& v : arr.vertex_handles()) {
        ...
        std::sort(heights.begin(), heights.end(),
                  [](hf_pair& a, hf_pair& b) { return a.first < b.first; });
      ...
      if (cfg.generate_walls) {
        for (auto edge : arr.edge_handles()) {
          ...
          vec3f v1_other =
              get_heights(vertex_columns[v1], v1, f_a, f_b, h1a, h1b);
          vec3f v2_other =
              get_heights(vertex_columns[v2], v2, f_a, f_b, h2a, h2b);

          // // set base (ground) elevation to vertices adjacent to a face
          // oustide the building fp
          int part_id;
          if (fp_a && !fp_b) {
          ...
          if ((h1a < h1b) and (h2a < h2b)) {
            ...
            // v1_other desc
            wall_face_1.insert(wall_face_1.end(), v1_other.rbegin(),
                               v1_other.rend());
            wall_face_1.push_back(v2p(v1, h1a));
            wall_face_1.push_back(v2p(v2, h2a));
            // v2_other asc
            wall_face_1.insert(wall_face_1.end(), v2_other.begin(),
                               v2_other.end());
          ...
          } else if ((h1a > h1b) and (h2a > h2b)) {
            wall_face_1.push_back(v2p(v2, h2a));
            // v2_other desc
            wall_face_1.insert(wall_face_1.end(), v2_other.rbegin(),
                               v2_other.rend());
            wall_face_1.push_back(v2p(v2, h2b));
            wall_face_1.push_back(v2p(v1, h1b));
            // v1_other asc
            wall_face_1.insert(wall_face_1.end(), v1_other.begin(),
                               v1_other.end());
            wall_face_1.push_back(v2p(v1, h1a));
            faces.push_back(wall_face_1);
            labels.push_back(wall_label);
            ...
            // compute vx and hx
            auto l_a = EPECK::Line_3(EPECK::Point_3(p1.x(), p1.y(), h1a),
                                     EPECK::Point_3(p2.x(), p2.y(), h2a));
            ...
            if (px != nullptr)
              ...
              throw roofer::rooferException(
                  "Invalid intersection point in Extruder");
            ...
            // v2_other desc
            wall_face_2.insert(wall_face_2.end(), v2_other.rbegin(),
                               v2_other.rend());
            ...
            wall_face_2.push_back(v2p(v2, h2a));
            // v2_other asc
            wall_face_2.insert(wall_face_2.end(), v2_other.begin(),
                               v2_other.end());
            wall_face_2.push_back(v2p(v2, h2b));
            wall_face_2.push_back(p2p(px));
    ...
    void compute(Arrangement_2& arr, float base_elevation,
                 ArrangementExtruderConfig cfg) override {
      auto const_elevation_provider_ptr =
          createElevationProvider(base_elevation);
