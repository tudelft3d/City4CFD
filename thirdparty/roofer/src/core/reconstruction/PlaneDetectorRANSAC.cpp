  ...
  struct PlaneDetectorRANSAC : public ShapeDetectorInterface {
    ...
    unsigned detectPlanes(PointCollection& point_collection, vec3f& normals,
      ...
      for (auto shape : ransac.shapes()) {
        ...
        // Plane
        // plane_cgal(boost::get<0>(pnl_points[plane.indices_of_assigned_points().front()]),
        // plane.plane_normal());
      ...
      for (auto& pt : points) {
        auto& p = std::get<0>(pt);
        auto& n = std::get<1>(pt);
        point_collection.push_back({float(CGAL::to_double(p.x())),
                                    float(CGAL::to_double(p.y())),
                                    float(CGAL::to_double(p.z()))});
        normals.push_back({float(CGAL::to_double(n.x())),
