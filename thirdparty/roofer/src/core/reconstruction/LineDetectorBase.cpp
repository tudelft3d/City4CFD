...
namespace roofer::linedect {
  ...
  LineDetector::LineDetector(vector<Point>& points) {
    ...
    size_t i = 0;
    for (auto p : points) indexed_points.push_back(std::make_pair(p, i++));
  ...
  vector<size_t> LineDetector::get_point_indices(size_t shape_id) {
    ...
    for (auto pi : indexed_points) {
      auto idx = pi.second;
      if (point_segment_idx[idx] == shape_id) result.push_back(idx);
    }
    return result;
  }
  ...
  SCK::Segment_3 LineDetector::project_cgal(const size_t i1, const size_t i2,
                                            float extension) {
    const auto& l = segment_shapes[point_segment_idx[i1]];
    const auto& p1 = indexed_points[i1].first;
    const auto& p2 = indexed_points[i2].first;
    ...
    return SCK::Segment_3(
        SCK::Point_3(CGAL::to_double(p1n.x()), CGAL::to_double(p1n.y()),
                     CGAL::to_double(p1n.z())),
        SCK::Point_3(CGAL::to_double(p2n.x()), CGAL::to_double(p2n.y()),
                     CGAL::to_double(p2n.z())));
  }
  ...
  size_t LineDetector::get_bounded_edges(roofer::SegmentCollection& edges) {
    ...
    for (auto seg : segment_shapes) {
      ...
      auto l_normal =
          l.to_vector() / CGAL::sqrt(l.to_vector().squared_length());
      for (auto id : l_idx) {
      ...
      ordered_segments[id_min][0] = {float(p0.x()), float(p0.y()),
                                     float(p0.z())};
      ordered_segments[id_min][1] = {float(p1.x()), float(p1.y()),
                                     float(p1.z())};
    }
  ...
  inline Line LineDetector::fit_line(vector<size_t>& neighbour_idx) {
    ...
    Line line;
    linear_least_squares_fitting_3(neighbor_points.begin(),
                                   neighbor_points.end(), line,
  ...
  std::vector<size_t> LineDetector::detect() {
    ...
    auto cmp = [](index_dist_pair left, index_dist_pair right) {
      return left.second < right.second;
    };
    priority_queue<index_dist_pair, vector<index_dist_pair>, decltype(cmp)> pq(
        cmp);
    ...
    while (pq.size() > 0) {
      ...
      if (point_segment_idx[idx] == 0) {
        if (grow_region(idx)) new_regions.push_back(region_counter);
        region_counter++;
      }
    }
    return new_regions;
  ...
  bool LineDetector::grow_region(size_t seed_idx) {
    ...
    while (candidates.size() > 0) {
      auto candidate_id = candidates.top();
      candidates.pop();
      auto cp = indexed_points[candidate_id].first;
      for (auto n_id : neighbours[candidate_id]) {
        if (point_segment_idx[n_id] != 0) continue;
        // point_seed_flags[n_id] = false; // this point can no longer be used
        // as seed
        if (valid_candidate(segment_shapes[region_counter],
                            indexed_points[n_id].first)) {
          ...
          Line line;
          linear_least_squares_fitting_3(points_in_region.begin(),
                                         points_in_region.end(), line,
                                         CGAL::Dimension_tag<0>());
          segment_shapes[region_counter] = line;
        }
      }
    ...
    if (points_in_region.size() < min_segment_count) {
      ...
      for (auto idx : idx_in_region) point_segment_idx[idx] = 0;
      return false;
    }
    return true;
  }
